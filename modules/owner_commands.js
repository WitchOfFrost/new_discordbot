...
module.exports.bash_execute = async function (obj_sub) {
    if (obj_sub.dc_msg.member.id == config.bot_config.owner) {
        ...
        await exec(`${obj_sub.dc_args.slice(1).join(" ")}`, puts)
    }
}
    ...
    if (obj_sub.dc_msg.member.id == config.bot_config.owner) {
        ...
        if (config.bot_config.allow_edit == "true") {
            ...
            fs.writeFile(fileName, JSON.stringify(file), function (err) {
                ...
                obj_sub.dc_msg.channel.send(`Edited config.json, set ${obj_sub.dc_args.slice(1, 2).toString()}.${obj_sub.dc_args.slice(2, 3).toString()} to ${obj_sub.dc_args.slice(3, 4).toString()}. If this was a mistake, please revert it asap!`)
